...
function Chatmate({ data, currentUserId }) {
  ...
  return (
    <>
      {userData &&
        (<div className={`cursor-pointer ${selected
          ? "bg-white ml-2 rounded-l-lg dark:bg-tahiti-300"
          : "bg-tahiti-100 rounded-none dark:bg-tahiti-200"
          } rounded-l-lg text-black dark:text-white flex items-center`} onClick={() => getConversation(userData)}>
          <div className='flex-none p-3 relative'>
            <span className={(isOnline ? 'bg-green-500' : 'bg-slate-700') + " rounded-full absolute left-0 w-3 h-3 ml-3 border-2 border-slate-400"}></span>
            <img
